...
-(void)loadFriends2{
    ...
    NSManagedObjectContext *context = [ZSRXMPPTool sharedZSRXMPPTool].rosterStorage.mainThreadManagedObjectContext;
    ...
    // 3.设置过滤和排序
    // 过滤当前登录用户的好友
    NSString *jid = [ZSRUserInfo sharedZSRUserInfo].jid;
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [[ZSRXMPPTool sharedZSRXMPPTool].roster removeUser:freindJid];
    }
}
